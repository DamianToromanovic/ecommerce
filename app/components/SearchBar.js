...
export default function SearchBar() {
  ...
  const filteredProducts = products
    .filter((p) => p.name.toLowerCase().includes(searchedTerm.toLowerCase()))
    .map((p) => ({
      type: "product",
      name: p.name,
  ...
  const filteredCategories = products
    .filter((p) =>
      p.category.toLowerCase().includes(searchedTerm.toLowerCase())
    )
    .map((p) => ({
      type: "category",
      name: p.category,
  ...
  const filteredSubcategories = products
    .filter((p) =>
      p.subcategory.toLowerCase().includes(searchedTerm.toLowerCase())
    )
    .map((p) => ({
      type: "subcategory",
      name: p.subcategory,
  ...
  return (
    <div className="md:flex w-full max-w-sm md:max-w-md relative hidden">
      <input
        ...
        className="flex-grow border border-gray-300 px-3 py-1 rounded-l-md focus:outline-none focus:ring-2 focus:ring-blue-400"
      />
      <Link href={`/search/${searchedTerm}`}>
        <button
          className="bg-blue-600 text-white px-4 py-2 text-sm rounded-r-md hover:bg-blue-700 whitespace-nowrap"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {filteredProducts.length === 0 ? (
                  ...
                  filteredProducts.slice(0, 4).map((p) => (
                    ...
                    >
                      <div className="flex items-center gap-4 border rounded-md p-3 hover:shadow cursor-pointer">
                        <img
                          src={p.image}
                          alt={p.name}
